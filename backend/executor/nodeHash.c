...
void ExecChooseHashTableSize(double ntuples, int tupwidth, bool useskew,
						...
						ExprContext *econtext)
{
	...
	if (hjstate->isSymHashJoin)
	{
		...
		if (hjstate->scanBucket)
		{
			...
			HashJoinTable hashtable = hjstate->hj_HashTable;
			HashJoinTuple hashTuple = hjstate->hj_CurTuple;
			uint32 hashvalue = hjstate->hj_CurOutHashValue;
			...
			while (hashTuple != NULL)
			{
				if (hashTuple->hashvalue == hashvalue)
				{
					...
					inntuple = ExecStoreMinimalTuple(HJTUPLE_MINTUPLE(hashTuple),
													 ...
													 false); /* do not pfree */
					// econtext->ecxt_outertuple = econtext->ecxt_innertuple;
		...
		else
		{
			...
			while (hashTuple != NULL)
			{
				if (hashTuple->hashvalue == hashvalue)
				{
					...
					if (ExecQualAndReset(hjclauses, econtext))
					{
						hjstate->hj_CurOutHashValue = hashTuple;
						return true;
					}
				}
...
bool ExecScanOutHashTableForUnmatched(HashJoinState *hjstate, ExprContext *econtext)
{
	HashJoinTable hashtable = hjstate->hj_outerHashTable;
	HashJoinTuple hashTuple = hjstate->hj_CurOutTuple;
